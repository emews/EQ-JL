...
# EQ-JL location
EQJL=$EMEWS_PROJECT_ROOT/ext/EQ-JL

# copy files from src to create canonical ext directory
mkdir -p $EQJL
SRC_ROOT=$EMEWS_PROJECT_ROOT/../../src/
cp $SRC_ROOT/*.swift $EQJL
mkdir -p $EQJL/emews
cp $SRC_ROOT/emews/*.jl $EQJL/emews
cp $SRC_ROOT/run_worker.sh $EQJL
...
turbine -n $PROCS $MACHINE $EMEWS_PROJECT_ROOT/swift/workflow.tic \
    -e TURBINE_OUTPUT \
    -e TURBINE_LOG \
    -e TURBINE_DEBUG \
    -e ADLB_DEBUG \
