...
(string results[]) dispatch(location locations[], file script_sh, int step, int arg_length, string std_out_dir, string data_dir) {
    ...
    // printf("JLMap.swift input_dir: %s", data_dir);
    foreach i in [0:max_idx] {
        string out_fname = "%s/out_%d.txt" % (std_out_dir, i);
        string err_fname = "%s/err_%d.txt" % (std_out_dir, i);
        // printf("JLMap.swift out_fname %s", out_fname);
        file out <out_fname>;
        file err <err_fname>;
        location l = locations[i];
        string result_f = "%s/result_%d.jld" % (data_dir, i);
        // printf("JLMap.swift %s\n", result_f);
        (out,err) = @location=l app_run_doe_worker(script_sh, i, total_chunks, step, data_dir) =>
        results[i] = result_f;
    }
}

(string result[]) run_jlmap(string payload[]) {
    // payload: jlmap,step,arg_length,rank_type
    printf("JLMap.swift run_jlmap");
    file script_sh = input(emews_root + "/ext/EQ-JL/run_worker.sh");
    string std_out_dir = "%s/jlmap" % turbine_output;
